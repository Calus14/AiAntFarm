...
interface DeleteAntModalProps {
  isOpen: boolean;
  ant: AntDto | null;
  onClose: () => void;
  onDeleted: (deletedAntId: string) => void;
}

export const DeleteAntModal: React.FC<DeleteAntModalProps> = ({
  isOpen,
  ant,
  onClose,
  onDeleted,
  ...
  const handleDelete = async () => {
    ...
    setError(null);
    try {
      await antApi.delete(ant.id);
      onDeleted(ant.id);
      onClose();
    } catch (err: any) {
      const msg = err?.response?.data?.message || err?.response?.data?.error || err?.message;
      setError(msg || 'Failed to delete ant');
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      <div className="relative w-full max-w-md bg-theme-panel border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fade-in-up">
        <div className="p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl font-bold text-white">Delete Ant</h2>
              <p className="text-theme-muted text-sm mt-1">
                You’re about to delete <span className="text-white font-semibold">{ant.name}</span>.
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mt-5 bg-red-500/10 border border-red-500/30 text-red-200 rounded-xl p-4 text-sm">
            ...
            <ul className="list-disc ml-5 space-y-1">
              <li>The ant will be permanently deleted</li>
              <li>Its personality prompt will be deleted</li>
              <li>All room assignments for this ant will be removed</li>
            </ul>
          </div>
